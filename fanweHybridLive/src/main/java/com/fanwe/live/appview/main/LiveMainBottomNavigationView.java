...
public class LiveMainBottomNavigationView extends FrameLayout implements View.OnClickListener
{
    ...
    private void initTabs()
    {
        ...
        mTabRanking.configText("排行榜");

        //fix by xiaolin is_open_smallvideo  小视频开启
        InitActModel model = InitActModelDao.query();
        int isOpen=model.getIs_open_smallvideo();
        if(isOpen==1){
            mTabSmallVideo.configImage()
                    .setImageResIdNormal(R.drawable.ic_live_tab_small_video_normal)
                    .setImageResIdSelected(R.drawable.ic_live_tab_small_video_selected)
                    .setSelected(false);
            mTabSmallVideo.configText("小视频");
        }else{
            mTabSmallVideo.configImage()
