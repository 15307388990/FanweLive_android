...
import com.fanwe.pay.room.RoomLiveNoticeViewerView;
...
public class LivePushViewerActivity extends LiveLayoutViewerExtendActivity implements LivePlayerSDK.TPlayCallback {
    ...
    private RoomLiveNoticeViewerView noticeViewerView;
    /**
     * 是否正在播放主播的加速拉流地址
     */
    ...
    protected void initLayout(View view) {
        ...
        fl_notice = (FrameLayout) view.findViewById(R.id.fl_notice);
        initNotice();
    }
    ...
    private void initNotice() {
        ...
        if (model.getVideo_notice_is_open() == 1) {
            noticeViewerView = new RoomLiveNoticeViewerView(this);
            noticeViewerView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (!TextUtils.isEmpty(model.getVideo_notice_url())) {
                        Intent intent = new Intent();
                        intent.setAction("android.intent.action.VIEW");
                        Uri content_url = Uri.parse(model.getVideo_notice_url());
                        intent.setData(content_url);
                        startActivity(intent);
                    }
                }
            });
            noticeViewerView.bindData(model.getVideo_notice_name());
            replaceView(fl_notice, noticeViewerView);
    ...
    public void onBsRequestRoomInfoSuccess(App_get_videoActModel actModel) {
       ...
       /* int is_live_pay=LiveInformation.getInstance().getRoomInfo().getIs_live_pay();
        if(is_live_pay==0){
            getViewerBusiness().startJoinRoom();//普通
        }else if(is_live_pay==1){
            // 0按时收费，1按场次收费,未开启收费模式,默认为2
           int  live_pay_type=LiveInformation.getInstance().getRoomInfo().getLive_pay_type();//收费类型
            if(live_pay_type==0){
                getTimePayViewerBusiness().startMonitor();//按时收费
            }else if(live_pay_type==1){
                //按场收费
                getScenePayViewerBusiness().requestSceneLivePayDeduct();
            }else if(live_pay_type==2){
                //不收费
                getViewerBusiness().startJoinRoom();//普通
            }
        }*/
